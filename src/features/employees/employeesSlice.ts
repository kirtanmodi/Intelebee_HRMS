...
export const selectActiveEmployees = (state: RootState) => 
  state.employees.list.filter(e => e.status === 'active');

export const selectEmployeeById = (id: string) => (state: RootState) =>
  state.employees.list.find(e => e.id === id);

export const selectTeamMembers = (managerId: string) => (state: RootState) =>
  state.employees.list.filter(e => e.managerId === managerId);
